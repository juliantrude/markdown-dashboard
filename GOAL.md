...
STATUS: READY
**Next action:** Increment 6 — widget framework: per-card toggle (icon buttons, top-right) including the faithful "Markdown" raw-render mode (real checkboxes for `- [ ]`, proper lists); selection persisted in localStorage, survives live reload, never mutates the file. Smoke: toggle switches the view and the choice sticks after reload.
**Blockers:** none
